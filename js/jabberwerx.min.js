 ...
 */
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/webinit.js*/
...
return jq;},createXMLDocument:function(){var doc;try{if(window.ActiveXObject){doc=new ActiveXObject("Msxml2.DOMDocument.3.0");doc.async=false;}else if(document.implementation&&document.implementation.createDocument){doc=document.implementation.createDocument(null,null,null);}else{throw new Error("No document constructor available.");}}catch(ex){jabberwerx.util.debug.log("Could not create XML Document: "+ex);throw ex;}
return doc;},parseXMLFromString:function(xmlstr){var tmp;try{try{tmp=jabberwerx.system.createXMLDocument();tmp.loadXML(xmlstr);}catch(ex){tmp=(new DOMParser()).parseFromString(xmlstr,"text/xml");}
tmp=tmp.documentElement;if(!tmp||(tmp.nodeName=="parsererror")||(jabberwerx.$("parsererror",tmp).length>0)){throw new TypeError("Parse error in jabberwerx.system.parseXMLFromString");}}catch(ex){jabberwerx.util.debug.log("exception: "+ex+" trying to parse xml: '"+xmlstr+"'");throw ex;}
return tmp;},serializeXMLToString:function(node)
{if(node&&(typeof(XMLSerializer)!="undefined")){return new XMLSerializer().serializeToString(node)}
return null;},setTimeout:function(func,delay){return window.setTimeout(func,delay);},clearTimeout:function(timeoutID){window.clearTimeout(timeoutID);},setInterval:function(func,delay){return window.setInterval(func,delay);},clearInterval:function(intervalID){window.clearInterval(intervalID);},getConsole:function(){return window.console||null;},getLocale:function(){return navigator.userLanguage||navigator.language;}};if(typeof(Node)!="undefined"&&Node.prototype&&Node.prototype.__defineGetter__){Node.prototype.__defineGetter__("xml",function(){return jabberwerx.system.serializeXMLToString(this);});}
jabberwerx.system.getLocation=function(){return(document&&document.location)||{};};window.jabberwerx=jabberwerx;})();
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/jabberwerx.js*/
;(function(jabberwerx){var jq=jabberwerx.system.jQuery_NoConflict();jabberwerx.$=jq;jabberwerx=jq.extend(jabberwerx,{version:'2011.16.31369',_config:{persistDuration:30,unsecureAllowed:false,capabilityFeatures:[],httpBindingURL:"/httpbinding",baseReconnectCountdown:30,enabledMechanisms:["DIGEST-MD5","PLAIN"]},_getInstallURL:function(){return this._getInstallPath();},_getInstallPath:function(){var p=this._config.installPath;if(!p){var target=String(arguments[0]||"jabberwerx")+".js";p=jabberwerx.$("script[src$='"+target+"']").slice(0,1).attr("src");p=p.substring(0,p.indexOf(target));}
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/jwapp/JWCore.js*/
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/jwapp/JWModel.js*/
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/util/crypt.js*/
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/model/Translator.js*/
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/model/Rendezvous.js*/
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/model/SASL.js*/
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/model/SASLMechs.js*/
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/model/JID.js*/
...
return 0;},shouldBeSerializedInline:function(){return false;},wasUnserialized:function(){if(jabberwerx.$.browser.msie){this.toString=function(){return this._full;}}
_jidCache[this.toString()]=this;},_node:"",_domain:"",_resource:"",_full:""},'jabberwerx.JID');jabberwerx.JID.InvalidJIDError=jabberwerx.util.Error.extend.call(TypeError,"The JID is invalid");jabberwerx.JID.asJID=function(val){var jid=null;if(val instanceof jabberwerx.JID){return val;}
...
return found;};return input.replace(ptn,repFN);};})(jabberwerx);
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/model/Events.js*/
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/model/Entity.js*/
...
return true;});return presence;},updatePresence:function(presence,quiet){var retVal=false;if(!presence){if(this.getPrimaryPresence()){var len=this._presenceList.length;for(var i=0;i<len;i++){var presence=this._presenceList.pop();!quiet&&this.event("resourcePresenceChanged").trigger({fullJid:presence.getFromJID(),presence:null,nowAvailable:false});}!quiet&&this.event("primaryPresenceChanged").trigger({presence:null,fullJid:this.jid.getBareJID()});retVal=true;}}else{if(!(presence instanceof jabberwerx.Presence)){throw new TypeError("presence is not a valid type");}
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/model/TemporaryEntity.js*/
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/model/Stanza.js*/
...
return 0;},_showStrToInt:function(showStr){showStr=(showStr?showStr:"");switch(showStr){case'chat':return 5;case'':return 4;case'dnd':return 3;case'away':return 2;case'xa':return 1;case'unavailable':return 0;default:return-1;}},setPresence:function(show,status,priority){if(show){this.setShow(show);}
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/model/User.js*/
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/model/Server.js*/
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/model/Stream.js*/
...
var url=jabberwerx.Stream.URI_PATTERN.exec(this._boshProps.httpBindingURL||"");if(!url){throw new TypeError("httpBindingURL not specified correctly");}
if(!url[1]){url[1]=jabberwerx.system.getLocation().protocol||"";}
...
this._boshProps.opening=jabberwerx.system.setTimeout(this.invocation("_handleOpen"),2000);this._boshProps.operation="reopen";this._sendRequest({restart:true});},close:function(){if(this.isOpen()){this._sendRequest({type:"terminate"},this._sendQ.empty());}else{this._reset();}},send:function(elem){if(!jabberwerx.isElement(elem)){throw new TypeError("elem must be a DOM element");}
if(!this.isOpen()){throw new jabberwerx.Stream.NotOpenError();}
this._sendQ.enque(elem);},_sendRequest:function(props,data){props=jabberwerx.$.extend({},this._boshProps,props);data=data||[];var rid=0,body;var resend=false;if(props.resend){rid=props.resend.id;body=jabberwerx.util.unserializeXML(props.resend.body);resend=true;data=jabberwerx.$(body).children();props.rid=rid+1;delete props.resend;}else{if(this._xhrs.size()>1&&data.length){this._sendQ.enque(data);return;}
...
body.attribute("xmlns:xmpp","urn:xmpp:xbosh").attribute("hold","1").attribute("ver","1.9").attribute("to",props.domain).attribute("wait",props.wait||30).attribute("{urn:xmpp:xbosh}xmpp:version","1.0");}else{body.attribute("sid",props.sid);if(props.restart){if(locale){body.attribute("xml:lang",locale);}
...
this._sendRequest({},this._sendQ.empty());},_prepareXHR:function(xhr,settings){this._xhrs.enque(xhr);},_handleXHR:function(xhr,status){if(this._dehydrated){return;}
...
/*build/dist/JabberWerxAJAX-release-2011.16.31369/src/model/Client.js*/
...
callback(elem.get()[0]);evt.notifier.unbind(arguments.callee);if(tid){jabberwerx.system.clearTimeout(tid);tid=undefined;}};timeout=Number(timeout||0);if(timeout>0){var tfn=function(){if(tid){that.event('beforeIqReceived').unbind(fn);var iq=i.errorReply(jabberwerx.Stanza.ERR_REMOTE_SERVER_TIMEOUT);iq.setFrom(to);callback(iq.getNode());}}
...
this.connectedUser=null;this.connectedServer=null;this._authMech=undefined;delete this._authMech;this._cleanupEntityCache();if(this._autoResourceName&&!this._countDownOn){this.resourceName=null;}
delete this._connectedRendezvous;this._stopReceiveQueue(true);this.setClientStatus(jabberwerx.Client.status_disconnected,err);},_handleElementsReceived:function(evt){var elements=jabberwerx.$(evt.data).get();this._stanzaRecvQ=this._stanzaRecvQ.concat(elements);jabberwerx.util.debug.log("client RECEIVED elements to process: "+
this._stanzaRecvQ.length,"rawStanzaLogging");this._startReceiveQueue(false);},_handleElementsSent:function(evt){var elements=jabberwerx.$(evt.data);for(var i=0;i<elements.length;++i){this._handleElementOut(elements.get(i));}},cancelReconnect:function(){if(this._reconnectTimerID!==null){jabberwerx.system.clearTimeout(this._reconnectTimerID);this._reconnectTimerID=null;this._countDownOn=false;this.event('reconnectCancelled').trigger();}},isSecure:function(){return this._stream.isSecure();},_startReconnectCountdown:function(){var base=jabberwerx._config.baseReconnectCountdown;if(base>0){var reconnectCountdown=base+Math.round((Math.random()-0.5)*base/5);this._reconnectTimerID=jabberwerx.system.setTimeout(this.invocation('_reconnectTimeoutHandler'),reconnectCountdown*1000);this._countDownOn=true;this.event('reconnectCountdownStarted').trigger(reconnectCountdown);}},_reconnectTimeoutHandler:function(){this._countDownOn=false;this._reconnectTimerID=null;this._connectParams.arg.reconnecting=true;this.connect(this._connectParams.jid,this._connectParams.password,this._connectParams.arg);},_handleElementOut:function(stanza){stanza=jabberwerx.Stanza.createWithNode(stanza);var stanzaType=stanza.pType();this.event(stanzaType+"Sent").trigger(stanza);},_startReceiveQueue:function(){if(this._stanzaRecvWorker||!this._stanzaRecvQ.length){return;}
...
if(clear){this._stanzaRecvQ=[];}},_processReceiveQueue:function(){var idx=0;var stanza;delete this._stanzaRecvWorker;for(idx=0;idx<jabberwerx.Client.STANZA_PROCESS_COUNT;idx++){stanza=this._stanzaRecvQ.shift();if(!stanza){return;}
stanza=jabberwerx.Stanza.createWithNode(stanza);var stanzaType=stanza.pType();var notifiers=[this.event('before'+stanzaType+'Received'),this.event(stanzaType+'Received'),this.event('after'+stanzaType+'Received')];var that=this;var handled=false;var fn=function(results){handled=handled||Boolean(results);if(!handled){if(notifiers.length){notifiers.shift().trigger(stanza,undefined,arguments.callee);}else{if(!results&&stanzaType=='iq'&&(stanza.getType()=='get'||stanza.getType()=='set')){stanza=stanza.errorReply(jabberwerx.Stanza.ERR_FEATURE_NOT_IMPLEMENTED);that.sendStanza(stanza);}}}};fn(false);}
